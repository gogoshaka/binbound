...
const systemPromptConversationScenario = `
- You are an english teacher and you are teaching englis to a student. 
...
In the JSON reply, provide the following information:
- Rephrase the dialog from the student in a more professional way
- Propose corrections (only on the last dialog from the user) based on syntax. Also:
  - Don't correct politeness
  - Don't correct punctuation
  - Don't correct anything related to the writing as what you read has been transcribed from speech.
...
export async function chatCompletion(chatHistory: MessageItemType[], lastContent: string) : Promise<LLMResponseToStudentType|null> {
  if (LLM === "OPENAI") {
...
export async function getOpenAIchatCompletion(chatHistory: MessageItemType[], lastContent: string) : Promise<LLMResponseToStudentType|null> {
...
export async function getGroqChatCompletion(chatHistory: MessageItemType[], lastContent: string) : Promise<LLMResponseToStudentType|null> {
  ...
  const  systemPrompt = `${systemPromptConversationScenario}
  - Provide answer as a json object with the following format: ${LLMResponseToStudentTypeJSON}`
  const response = await groq.chat.completions.create({
    ...
    return JSON.parse(response.choices[0]?.message?.content) as LLMResponseToStudentType
